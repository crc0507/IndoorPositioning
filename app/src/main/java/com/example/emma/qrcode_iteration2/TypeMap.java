 ...
 * 介绍poi周边搜索功能
 */
public class TypeMap extends Activity implements OnClickListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        shopcode = bundle2.getString("typecode");

        Intent intent = getIntent();
        Bundle bundle = intent.getExtras();
        latitudex = bundle.getDouble("jingjing");
        longitudey = bundle.getDouble("weiwei");
...
//		keyWord = mSearchText.getText().toString().trim();
        currentPage = 0;
        query = new PoiSearch.Query("", shopcode, "010");// 第一个参数表示搜索字符串，第二个参数表示poi搜索类型，第三个参数表示poi搜索区域（空字符串代表全国）
...
//                        intent = new Intent(TypeMap.this, ShopList.class);
//                        List<PoiData> list = getData(poiItems);
//                        intent.putExtra("poiitemslist", (Serializable)getData(poiItems));
//                        startActivity(intent);

                        for(int i=0;i<poiItems.size();i++){
                            double a = poiItems.get(i).getLatLonPoint().getLatitude();
                            double b = poiItems.get(i).getLatLonPoint().getLongitude();
                        ...
                        //ToastUtil.show(TypeMap.this, poiItems.size()+"");
                        /////////////gai
                        //用来测试的toast显示
...
//                        locationMarker.showInfoWindow();
                        ...
                        ToastUtil.show(TypeMap.this,"this city no");
                    } else {
                        ToastUtil.show(TypeMap.this,
                                "error");
                    }
                }
            } else {
                ToastUtil.show(TypeMap.this, "error");
            }
        } else  {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.checktype:
                ...
                intent = new Intent(TypeMap.this, ShopList.class);
                List<PoiData> list = getData(poiItems);
                intent.putExtra("poiitemslist", (Serializable)list);
                startActivity(intent);
                break;
            default:
                break;
